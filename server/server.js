...
app.post("/auth", function (req, res) {
  ...
  connection.query(query, function (error, results, fields) {
    if (error) {
      res.send(
        JSON.stringify({ message: "ошибка выполнения запроса", error: 0 })
      );
    }
...
app.post("/services", function (req, res) {
  const query =
    'INSERT INTO `services`(`clinic_id`, `name`, `cost`, `duration`) VALUES (1,"' +
    req.body.service +
    '",' +
    req.body.cost +
    "," +
...
app.post("/stuff", function (req, res) {
  const query =
    'INSERT INTO `stuff`(`clinic_id`, `fio`, `spec`, `time`) VALUES (1,"' +
    req.body.fio +
    '","' +
    req.body.spec +
    '",' +
...
app.post("/records", function (req, res) {
  ...
  connection.query(query, function (error, results, fields) {
    if (error) {
      res.send(
        JSON.stringify({ message: "непредвиденная ошибка", error: error })
      );
    }
    res.send(JSON.stringify({ message: "ok" })); // результат из result можно запихнуть в ответ
  });
